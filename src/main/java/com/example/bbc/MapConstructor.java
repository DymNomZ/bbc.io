...
public class MapConstructor {
    ...
    private int getVerifiedPosition(int position, int oppositePosition, int size, int oppositeSize, int deltaPosition, boolean isXDimension) {
        ...
        int maxSpeedPerLoop = GameScene.TILE_SIZE;
        if (deltaPosition < 0) {
            maxSpeedPerLoop *= -1;
        }
        int maxPixelOnMap = (isXDimension ? map_length : map_height) * GameScene.TILE_SIZE;
        int offPixelOnTile = GameScene.TILE_SIZE;
        if (size % GameScene.TILE_SIZE == 0) {
            if (size > GameScene.TILE_SIZE) {
                offPixelOnTile -= GameScene.TILE_SIZE;
            } else {
                offPixelOnTile -= size;
            }
        } else {
            offPixelOnTile -= size % GameScene.TILE_SIZE;
        }

        int oppositeOffTile = oppositeSize % GameScene.TILE_SIZE;
        boolean isBetween = oppositePosition % GameScene.TILE_SIZE > (oppositeOffTile == 0 ? 0 : GameScene.TILE_SIZE - oppositeOffTile);
        int loopN = oppositeSize / GameScene.TILE_SIZE;
        ...
        if (isXDimension) {
            initOffYTile = oppositePosition / GameScene.TILE_SIZE;
            nextOffYTile = 1;
        } else {
            initOffXTile = oppositePosition / GameScene.TILE_SIZE;
            nextOffXTile = 1;
        }
        ...
        while (deltaPosition != 0) {
            ...
            int pixelsOnTile = position % GameScene.TILE_SIZE;
            ...
            if (position + deltaTemp < 0) {
                ...
                deltaPosition = 0;
            } else if (offTile >= 0 && ((pixelsOnTile < offPixelOnTile && offTile <= offPixelOnTile) || (pixelsOnTile > offPixelOnTile && offTile <= GameScene.TILE_SIZE + offPixelOnTile))) {
                position += deltaTemp;
            } else {
                ...
                if (offTile >= 0) {
                    if (isXDimension) {
                        offXTile = (position + deltaTemp + size) / GameScene.TILE_SIZE;
                    } else {
                        offYTile = (position + deltaTemp + size) / GameScene.TILE_SIZE;
                    }
                } else {
                    if (isXDimension) {
                        offXTile = (position + deltaTemp) / GameScene.TILE_SIZE;
                    } else {
                        offYTile = (position + deltaTemp) / GameScene.TILE_SIZE;
                    }
                }
                    ...
                    if (tiles[offYTile][offXTile].is_solid) {
                        ...
                        if (offTile < 0) {
                            ...
                            if (pixelsOnTile == offPixelOnTile) {
                                ...
                                position -= offTile - offPixelOnTile - GameScene.TILE_SIZE;
                            } else {
    ...
    public void view(int x, int y, CameraEntity camera) {
        ...
        camera.x = Math.min(camera.x, (map_length * GameScene.TILE_SIZE) - camera.width);

        camera.y = Math.max(y, 0);
        camera.y = Math.min(camera.y, (map_height * GameScene.TILE_SIZE) - camera.height);
    }
    ...
    public void displayTiles(Graphics g, CameraEntity camera) {
        int leftStart = Math.max((camera.x / GameScene.TILE_SIZE) - 1, 0);
        int topStart = Math.max((camera.y / GameScene.TILE_SIZE) - 1, 0);
        int rightEnd = Math.min(((camera.x + camera.width) / GameScene.TILE_SIZE) + 1, map_length);
        int bottomEnd = Math.min(((camera.y + camera.height) / GameScene.TILE_SIZE) + 1, map_height);

        while (topStart < bottomEnd) {
            for (int i = leftStart; i < rightEnd; i++) {
                int tileX = (i * GameScene.TILE_SIZE) - camera.x;
                int tileY = (topStart * GameScene.TILE_SIZE) - camera.y;

                tiles[topStart][i].image.display(g, tileX, tileY, GameScene.TILE_SIZE, GameScene.TILE_SIZE);
            }
            topStart++;
        }
    }
